...
3. Fase 1 pivotada para automação do Outlook Web via Playwright (`app_facilitador/browser_client.py`). Login manual salvo em `scripts/browser_login.py`. **Em andamento — calibrando os seletores reais da caixa de entrada com `scripts/browser_inbox_debug.py`, rodado localmente pelo usuário (a automação de navegador com login real precisa rodar na máquina do usuário, não em ambiente headless de CI/agente).**
