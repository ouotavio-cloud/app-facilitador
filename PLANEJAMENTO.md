...
- **Opção A (IMAP usuário/senha)**: descartada. A Microsoft desativou por padrão o "Basic Auth" em contas Microsoft 365 corporativas desde 2022. Houve um script de diagnóstico (`scripts/test_imap.py`) para confirmar se esta conta era exceção; ele foi removido na v17, junto com o resto do código do caminho que não se usa.
...
2. ~~Registrar App no Azure AD~~ — descartado (bloqueado por política de TI); usando client_id público da Microsoft (seção 2). **Feito**, mas depois **superado**: a própria Graph API foi bloqueada pelo tenant (ver seção 2, Atualização 1), e o caminho ativo passou a ser a automação do Outlook Web (Opção D). A autenticação MSAL (`app_facilitador/auth.py`, `app_facilitador/graph_client.py`) ficou um tempo como "código de referência" e foi **removida na v17**: nada a chamava, e ela ainda era empacotada dentro do `.exe` junto com `msal` e `requests`. O histórico do git guarda a implementação, se um dia a TI liberar a API.
...
12. **Distribuição como executável (Fase 7)** — o app virou um `AppFacilitador.exe` único, baixável de uma release do GitHub. Três decisões sustentam isso:
    ...
    - **Navegador: o Chromium empacotado.** Chegamos a usar o Edge instalado no Windows para economizar ~150 MB no download. Foi um erro, revertido depois do teste com o usuário: no Windows corporativo dele o Edge **não preservava a conta entre execuções** — era preciso logar de novo o tempo todo — enquanto o Chromium mantinha. Economizar download não vale um app que pede senha toda hora. O tamanho também mudou a forma de distribuir: com o navegador junto, o modo "arquivo único" descompactaria 150 MB a cada abertura, então o app virou uma pasta compactada em zip.
...
14. **Perfil de navegador persistente**, em vez de `storage_state.json`. O arquivo de estado guarda cookies e localStorage, mas não o IndexedDB — onde o login da Microsoft deixa parte do que precisa. O sintoma relatado pelo usuário foi a conta parar de valer pouco depois. Um perfil de navegador de verdade (`launch_persistent_context`) preserva tudo, e a sessão dura o mesmo que duraria no navegador do dia a dia. Como efeito colateral, o código ficou mais simples: não há mais um arquivo de estado para salvar e recarregar, só uma pasta que o navegador administra.
