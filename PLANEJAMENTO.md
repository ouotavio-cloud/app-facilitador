...
3. Fase 1 pivotada para automação do Outlook Web via Playwright (`app_facilitador/browser_client.py`). Login manual salvo em `scripts/browser_login.py`. **Seletores calibrados com dados reais da caixa de entrada** (via `scripts/browser_inbox_debug.py`, rodado localmente pelo usuário — a automação de navegador com login real precisa rodar na máquina do usuário, não em ambiente headless de CI/agente). `list_visible_messages()` já extrai remetente, e-mail, assunto, data/hora e preview de forma estruturada, com testes de regressão (`tests/test_browser_client.py`) usando os dados reais capturados. **Falta:** varrer a caixa completa com scroll (só cobre os itens visíveis na tela por enquanto — ver seção 1.3) e abrir a mensagem/anexo de fato (a extração hoje é só da lista, não do corpo completo nem do anexo).
4. Fase 2 (motor de identificação de propostas) iniciada em paralelo, por não depender da calibração dos seletores: `app_facilitador/proposal_detector.py` reconhece o código `SUP.AAAA-NNN` com variações de formatação (ponto, espaço, hífen opcionais) via regex configurável (`config.PROPOSAL_CODE_PATTERNS`). **Feito o parser de código; falta ligar à extração de texto de assunto/corpo/anexo reais.**
